...
def entropia(df_dados:pd.DataFrame, nom_col_classe:str) -> float:
    ...
    for val_atr,count_atr in ser_count_col.iteritems():
        ...
        entropia -= math.log2(val_prob)*val_prob
    return entropia
...
def ganho_informacao_condicional(df_dados: pd.DataFrame, val_entropia_y:float, nom_col_classe:str, nom_atributo:str, val_atributo:Union[int,float,str,bool]) ->float:
    ...
    df_dados_filtrado = df_dados.loc[df_dados[nom_atributo] == val_atributo]
...
def ganho_informacao(df_dados:pd.DataFrame, nom_col_classe:str, nom_atributo:str) -> float:
    ...
    val_info_gain = 0
    for val_atr,count_atr in ser_count_col.iteritems():
        val_prob = ganho_informacao_condicional(
            df_dados, val_entropia_y, nom_col_classe, nom_atributo, val_atr)
        val_info_gain += count_atr/num_total * val_prob
